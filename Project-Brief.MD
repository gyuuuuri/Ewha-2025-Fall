...
|Pain Point|생성형 AI의 확산으로 데이터 센터 에너지 사용 급증/GPU 연산 집중, 대규모 파라미터 이동, 고빈도 I/O로 기존 워크로드와는 상이한 자원별(CPU, GPU, 메모리, 스토리지) 에너지 사용 패턴 및 에너지 병목 발생|
|사용할 소프트웨어 패키지의 명칭과 핵심기능/용도, 사용시나리오|Python 3.8+ : 메인 개발 언어, PyTorch : AI 모델 훈련/추론, scikit-learn : 머신러닝 모델 구현,  pandas/numpy : 데이터 처리 및 분석, pynvml : NVIDIA GPU 전력 모니터링|
|사용할 소프트웨어 패키지의 명칭과 URL|PyTorch: (https://pytorch.org/),scikit-learn: (https://scikit-learn.org/),pynvml: (https://pypi.org/project/pynvml/)|
